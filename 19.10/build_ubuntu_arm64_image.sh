#!/bin/sh
# Generate a 64-bit Ubuntu Server 19.10 (Eoan Ermine) image for the Raspberry Pi 4.
#
...
LATEST_EOAN=$(curl http://cdimage.ubuntu.com/ubuntu/releases/eoan/release/ | grep "arm64+raspi3.img.xz\"" | head -n 1 | cut -d '"' -f 2)

if !(test -e $LATEST_EOAN); then
	wget http://cdimage.ubuntu.com/ubuntu/releases/eoan/release/$LATEST_EOAN
	if test $? -ne 0; then
		exit $?
	fi
	curl http://cdimage.ubuntu.com/ubuntu/releases/eoan/release/SHA256SUMS | grep arm64+raspi3 > $LATEST_EOAN.sha256
	if test $? -ne 0; then
		exit $?
	fi
fi

sha256sum -c "$LATEST_EOAN".sha256
if test $? -ne 0; then
	exit $?
fi

MODIFIED_IMAGE=$(echo "$LATEST_EOAN" | cut -d '+' -f 1)+rpi4.img

xzcat $LATEST_EOAN > $MODIFIED_IMAGE
if test $? -ne 0; then
	exit $?
fi
